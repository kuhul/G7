...
'------------------------------------------------------------------------------- Konstanten
const piover180 = 0.0174532925f ' f�r trigo-Fkt
const sichtweite = 10 '25 '9
const stepphi = 4
const steptheta = 2
const stepforward = 3.0 ' 0.1
...
const screenx = 1900., screeny=1000. ' Aufl�sung Gamefenster
...
declare sub PlayerKeyUp(byval heading as single, byval deltaTime as single, byref deltax as single,_
						byref deltay as single, byref deltaz as single)
declare sub PlayerKeyDown(byval heading as single, byval deltaTime as single, byref deltax as single,_
						  byref deltay as single,byref deltaz as single)
declare sub PlayerKeyLeft(byval heading as single, byval deltaTime as single, byref deltax as single,_
						  byref deltay as single,byref deltaz as single)
declare sub PlayerKeyRight(byval heading as single, byval deltaTime as single, byref deltax as single,_
						   byref deltay as single, byref deltaz as single)
...
dim shared as integer animationTimer = 0 'Z�hler um zum n�chsten Animationsframe zu switchen
 ...
 '------------------------------------------------------------------------------- MAINLOOP
  do
    ...
    if (hp<0) then
      ...
      if MULTIKEY(FB.SC_W) then PlayerKeyUp(yrot, deltaTime, deltax,deltay,deltaz)
      if MULTIKEY(FB.SC_S) then PlayerKeyDown(yrot, deltaTime, deltax,deltay,deltaz)
      if MULTIKEY(FB.SC_A) then PlayerKeyLeft(yrot, deltaTime, deltax,deltay,deltaz)
	  if MULTIKEY(FB.SC_D) then PlayerKeyRight(yrot, deltaTime, deltax,deltay,deltaz)
    ...
    ' Spielerwaffe zeichnen falls gez�ckt
...
'    'l�scht alle X-Marker auf der Map und setzt dann neue
'    for y = 1 to 64
'        for x = 1 to 64
...
'------------------------------------------------------------------------------- getMonsterIndexAt
' Gibt den index des Monsters an der position xpos,ypos zur�ck oder -1
...
'------------------------------------------------------------------------------- PlayerKeyUp
' Key-Up Action
sub PlayerKeyUp(byval heading as single, byval deltaTime as single, byref deltax as single,_
				byref deltay as single, byref deltaz as single)
	deltaz = -sin(heading*piover180) * stepforward * deltaTime
	deltax = -cos(heading*piover180) * stepforward * deltaTime
	walkbiasangle = winkeladd(walkbiasangle,10.)  'Laufbewegung
	deltay = sin(walkbiasangle * piover180)*bounce
end sub

'------------------------------------------------------------------------------- PlayerKeyDown
sub PlayerKeyDown(byval heading as single, byval deltaTime as single, byref deltax as single,_
				  byref deltay as single, byref deltaz as single)
	deltaz = sin(heading*piover180) * stepforward * deltaTime
	deltax = cos(heading*piover180) * stepforward * deltaTime
	walkbiasangle = winkeladd(walkbiasangle,-10.)  'Laufbewegung
...
'------------------------------------------------------------------------------- PlayerKeyLeft
sub PlayerKeyLeft(byval heading as single, byval deltaTime as single, byref deltax as single,_
				  byref deltay as single, byref deltaz as single)
	deltaz = -sin((heading+90)*piover180) * stepforward * deltaTime
	deltax = -cos((heading+90)*piover180) * stepforward * deltaTime
end sub

'------------------------------------------------------------------------------- PlayerKeyRight
sub PlayerKeyRight(byval heading as single, byval deltaTime as single, byref deltax as single,_
				   byref deltay as single, byref deltaz as single)
	deltaz = -sin((heading-90)*piover180) * stepforward * deltaTime
	deltax = -cos((heading-90)*piover180) * stepforward * deltaTime
end sub
...
'------------------------------------------------------------------------------- FindSteinId
' gibt die zum Stein xp,yp geh�rigen Stein zur�ck
...
sub drawobject(byval goffx as single, byval goffy as single, byval l1 as single, byval l2 as single,_
  ...
  elseif (ang<>0 and ang2=0) then
    glTranslatef l1, level-1+ztol, l2    ' Spielfeldkoordinaten
    glTranslatef 0.5, 0., 0.5  ' Mitte Element f�r Drehung rueckg�ngig
