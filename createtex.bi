...
'------------------------------------------------------------------------
'' Create texture with the fbpng lib
private function CreateTexturePNG(byval filename as string,_
                               byval flags as integer = 0 ) as uinteger
    dim as integer w, h
    dim tex as uinteger
    dim buffer as fb.Image ptr
	dim as GLenum formata, minfilter, magfilter
    
    buffer = png_load("UI.png", PNG_TARGET_OPENGL)
...
'    magfilter = GL_LINEAR
'    minfilter = GL_LINEAR
'    glTexImage2D(GL_TEXTURE_2D, 0, formata, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, buffer)
'    '########################'
	...
	if(flags and TEX_MIPMAP) then
		gluBuild2DMipmaps(GL_TEXTURE_2D, formata, w, h, GL_RGBA, _
		...
		glTexImage2D(GL_TEXTURE_2D, 0, formata, w, h, 0, GL_RGBA, _
                    GL_UNSIGNED_BYTE, buffer)
		minfilter = magfilter
	end if
